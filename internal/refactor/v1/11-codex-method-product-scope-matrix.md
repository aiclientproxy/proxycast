...
| 状态                     | 数量 | 裁决                                                                                                                                                                                                              |
| ------------------------ | ---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `implemented`            |   69 | 连接握手、核心 Thread/Turn/Item、thread subscription/lifecycle/content search/raw item injection/background terminals/elicitation/Guardian continuation、typed approval/MCP server request 与 model control plane |
| `planned`                |  111 | process/fs、config、model reroute/verification、Hooks、Skills/Plugins/Apps、review 与 Windows sandbox                                                                                                             |
...
blocker，但不影响这两个 method boundary 的 implemented 分类。下一刀回到多模型控制平面，优先补
`model/rerouted`、`model/verification` 与 provider readiness/retry evidence，再处理
...
`69 / 180 = 38.3%`。分母较上一 revision 增加 `1`，来自 Codex HEAD 新增的
