...
| v2 notification | 72 项 coverage、分离的 pinned Item schema/current method revision、unknown recorder，以及 typed `configWarning` / thread-scoped `warning` producer、GUI 与 warning recovery 已落地；experimental fuzzy session notification 仅进入脱敏 diagnostics | 其余 planned notification 尚无完整 producer、GUI outlet 与 Gate B                           |
