...
该结果只完成 P1 核心 identity/dynamic-tool round trip，不等于完整 Gate B。旧 `right-surface-visual-matrix` 仍是 Canvas/CDP fixture，不能作为 Gate A/B；approval、cancel、disconnect、window close、权限/下载、直接 destroyed WebContents 和历史恢复仍需扩展真实 Electron evidence。
...
Electron Host 现已把 sidecar stop、退出、重启失败和 stale connection 统一映射为 Browser disconnect cleanup：user-origin tab release 并 detach debugger，Agent-origin 无 mark tab close；native `destroyed` 与 `render-process-gone` 均回收 route 并发出 close event。该行为已有 32 项 Electron Host 定向单测和 `typecheck:electron` 证据；真实 Gate B 已覆盖 renderer-process-gone，disconnect/window-close 仍待补真实证据。

同日验证收口：`npm run test:contracts` 已取得明确退出码 `0`。最新 Browser Electron Gate B 已以退出码 `0` 完成 active-turn 与 renderer termination 场景；25 项断言、console/page error 均通过，`typecheck:electron` 已随 fixture build 通过。此前 `npm run verify:gui-smoke` 的 SHELL-01 产品断言虽为 21/21，但 launcher 生命周期仍以 watchdog 退出码 `1`；`npm run smoke:agent-runtime-current-fixture` 仍在既有 `unknown-item` Electron 场景超过 240 秒未收口。两项属于独立 harness 阻塞，不推翻 Browser Gate B，但在修复并重跑前不能宣称完整本地门禁全绿。

Browser terminal cleanup 已增加 fail-closed 状态门禁：Electron Host 只接受带有终态 status 的 `turn/completed` 通知来 release/close Browser route，`inProgress` 或缺少 status 的异常通知不会提前释放控制；该边界已有 62 项 Electron 定向回归覆盖。真实 Electron cancel 仍需补充 `turn/interrupt -> read model terminal -> Browser route` 的端到端证据。
...
- approval/takeover/deny/resume/interruption 形成真实闭环。
- 上传、下载、clipboard、权限和敏感数据使用 turn-scoped grant 与 artifact ref。
