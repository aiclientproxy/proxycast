...
- 未完成：P2-B 下一刀回到默认 command/test sandbox-aware process runner/control owner；外部 Agent host 消费 `process_control` 合同只在真实生产 host adapter 出现时继续；Windows restricted-token runner 尚未实现，Windows smoke 和平台 evidence 必须等 runner 进入 `Ready/enforced=true` 后再补；network / policy 单条规则深链属于 P7/P8 增强。P3/P4 UI evidence、Provider slot diagnostics 与 GUI coding input baseline 已进入 current 骨架闭环，不再作为主线 blocker。
...
- 2026-08-10 current 代码复核更正：以下 2026-06-15 记录指向已删除的旧 owner、旧测试入口，并曾把 Windows plan 写成 `ready/enforced=true`，因此只保留为 superseded historical evidence，不得作为 current owner 或平台交付证据。当前 runner foundation 已在 `tool-runtime::execution_process::windows` 重建并由 typed `LocalExecutionSandbox` 接线；`prepare_sandbox_command` 拒绝 `RestrictedToken` 是因为该 backend 走专用进程 runner，不是 runner 缺失。`plan_sandbox_backend` 仍返回 `Planned/enforced=false` 和 `sandbox_backend_windows_runner_platform_evidence_pending`，因为尚无 Windows/MSVC 编译、真机 enforcement 与 Desktop Gate B 证据。
