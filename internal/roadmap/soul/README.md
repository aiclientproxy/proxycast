...
2. [../memory/architecture.md](../memory/architecture.md)
3. [../../aiprompts/governance.md](../../aiprompts/governance.md)
4. [../../aiprompts/commands.md](../../aiprompts/commands.md)
...
1. 工具 lifecycle、协作执行、artifact/export、media/contentParts 和图片生成块已有 current metadata 骨架。
...
3. Media/contentParts 的大型媒体 preview policy、page-window 读取、Workbench 分页、server-side cancellation、`refId/sourceUri/relativePath-only` owner normalization、artifact/cache media owner 和 direct live shell cancel/terminate 骨架已进入 current owner。`media/read` 只保留 canonical `threadId` + bounded range response；前端在每段 response 通过范围、MIME、totalBytes 与 digest 校验后累计 progress，并继续支持 pagination/object URL/metadata fallback。V2-05 已物理删除 MediaRead `stream`、chunk/completed transient notification、raw subscription、response envelope、streaming parser 与 live drain，contract guard 禁止恢复第二数据通道。
4. Agent Plugin 的安装、激活、Skills、MCP 与 Apps 归 `internal/roadmap/plugin/v3/`；Soul 只消费标准 tool lifecycle facts，不拥有 Plugin 私有 inventory、runtime host 或专属五语文案。
...
9. [全局架构](../../aiprompts/architecture.md)与[持久化地图](../../aiprompts/persistence-map.md)已承接 media / contentParts current 骨架：`agent_message.contentParts.media` 从 App Server read model 进入 GUI media reference card，`sourcePath` 已通过 Electron `asset://` current fixture 显示真实图片，inline `data:` owner fail closed。唯一 sidecar reader 是 v2 `media/read`：按 canonical `threadId` 执行 bounded range、max-bytes、full-file sha256 与 request-id cancellation；GUI 只有在每段 response 通过 offset/length/MIME/totalBytes/digest 校验后才累计 progress，并负责 object URL budget/lifecycle、分页或 metadata fallback。artifact/cache media sidecar alias、media task data/file/workspace-local/受控远程输出写入 SidecarStore、tool/task owner facts 到 synthetic `contentParts.media` 投影均沿用 current owner。V2-05 已删除 MediaRead `stream`、chunk/completed transient notification、raw subscription、response notification envelope、streaming parser 和 live-drain helper，禁止恢复第二事件通道。该项仍不代表其它网络长 IO、live Provider 或跨重启恢复证据完成。
...
2026-07-31 cutover：媒体 sidecar 读取继续以 Lime-owned v2 `media/read` 为唯一生产入口；公共请求、响应、Renderer gateway 与 package client 只使用 canonical `threadId` 和 bounded range 参数。`MediaReadParams.stream`、`media.read.chunk` / `media.read.completed`、App Server streaming producer、客户端 raw subscription、response notification envelope 与 Renderer live-drain 已物理删除，progress 只由已校验 response 累计。`media-reference` 真实 Electron fixture 与独立 GUI smoke 均通过；这一刀只关闭 V2-05 media transient bypass，不代表其它 planned notification、host capability 或 recovery 已完成。
...
3. Refactor/v1 media 后续项：direct live shell process cancel/terminate 与 artifact/cache media owner 读取骨架已进入 current；后续补其它网络长 IO / live Provider owner 证据、Playwright 自然度验收和业务协作入口盘点。Plugin 标准化只由 `internal/roadmap/plugin/v3/` 继续推进。
