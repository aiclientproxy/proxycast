...
2026-07-29 cutover：媒体 sidecar 读取的唯一生产入口已从 retired v0 method 迁为 Lime-owned v2 `media/read`。公共请求、响应、Renderer gateway 与 package client 只使用 canonical `threadId`，前端 owner 固定为 `buildMediaReadParams(...)` / `readMedia(...)`；App Server 仅在内部把 thread 解析为真实 session 以约束 SidecarStore 物理路径。`media-reference` 真实 Electron Gate B 已覆盖 preload、`app_server_handle_json_lines`、成功图片预览、sidecar 不可用后的用户可见 metadata fallback，以及无 `sessionId` 回流。`media.read.chunk` / `media.read.completed` 仍是 deprecated transient notification 旁路，继续由 V2-05 迁出，本次 cutover 不代表 notification 已全面收口。
