...
export const PLUGIN_PACKAGE_MCP_APP_RESOURCE_URI =
  "ui://mcp-elicitation-plugin/release-check.html";
export const PLUGIN_PACKAGE_MCP_APP_TITLE = "Plugin package Release Check";
export const PLUGIN_PACKAGE_MCP_APP_MARKER = "PLUGIN_PACKAGE_MCP_APP_GATE_B_READY";

export function pluginPackageMcpAppHtml() {
  return `<!doctype html>
<html>
  <head><meta charset="utf-8"><title>${PLUGIN_PACKAGE_MCP_APP_TITLE}</title></head>
  <body data-plugin-package-mcp-app="${PLUGIN_PACKAGE_MCP_APP_MARKER}">
    <main><h1>${PLUGIN_PACKAGE_MCP_APP_TITLE}</h1><p>${PLUGIN_PACKAGE_MCP_APP_MARKER}</p></main>
  </body>
</html>`;
}

export function findPluginPackageMcpAppItems(value, items = []) {
  if (Array.isArray(value)) {
    value.forEach((entry) => findPluginPackageMcpAppItems(entry, items));
    return items;
  }
  if (!value || typeof value !== "object") return items;
  if (
    value.type === "mcpToolCall" &&
    value.mcpAppResourceUri === PLUGIN_PACKAGE_MCP_APP_RESOURCE_URI
  ) {
    items.push(value);
  }
  Object.values(value).forEach((entry) =>
    findPluginPackageMcpAppItems(entry, items),
  );
  return items;
}

export function pluginPackageMcpAppTraceEvidence(traceRaw, runtime) {
  ...
  const resourceReadAttempts = trace.flatMap((entry) => {
    ...
    if (
      ...
      request?.params?.uri === PLUGIN_PACKAGE_MCP_APP_RESOURCE_URI
    ) {
      return [
        {
          error:
  ...
  const htmlLoads = embeddedBrowserAttempts.filter(
    (entry) =>
      ...
      entry.source === "mcpApp" &&
      entry.sourceUri === PLUGIN_PACKAGE_MCP_APP_RESOURCE_URI,
  );
  return {
...
export async function installPluginPackageEmbeddedBrowserLifecycleCapture(page) {
  await page.evaluate(() => {
    ...
    const listenerKey = "__LIME_PLUGIN_PACKAGE_EMBEDDED_BROWSER_UNLISTEN__";
...
export async function waitForPluginPackageMcpAppSurface({
  app,
  page,
  options,
  runtime,
  ...
  while (Date.now() - startedAt < Math.min(options.timeoutMs, 60_000)) {
    ...
    latestTraceEvidence = pluginPackageMcpAppTraceEvidence(traceRaw, runtime);
    if (
      latest.some(
        (entry) =>
          entry.title === PLUGIN_PACKAGE_MCP_APP_TITLE &&
          entry.marker === PLUGIN_PACKAGE_MCP_APP_MARKER,
      )
    ) {
      if (
  ...
  throw new Error(
    `Plugin package MCP App surface 未完成: ${JSON.stringify({
      renderer,
      traceEvidence: latestTraceEvidence,
      webContents: latest,
    })}`,
...
export async function waitForPluginPackageMcpAppHistoryUnavailable({
  app,
  page,
  options,
  runtime,
}) {
  const { containerId, frame, viewId } = await waitForPluginPackageMcpAppFrame({
    page,
    options,
    runtime,
  });
  ...
  const traceEvidence = pluginPackageMcpAppTraceEvidence(traceRaw, runtime);
...
async function waitForPluginPackageMcpAppFrame({ page, options, runtime }) {
...
async function readMcpAppRendererDiagnostics(page, runtime) {
  ...
  return await page.evaluate(
    ({ itemId, itemPage, resourceUri, sessionId, threadId }) => {
      ...
      return {
        ...
        embeddedBrowserLifecycle: Array.isArray(
          window.__LIME_PLUGIN_PACKAGE_EMBEDDED_BROWSER_LIFECYCLE__,
        )
          ? window.__LIME_PLUGIN_PACKAGE_EMBEDDED_BROWSER_LIFECYCLE__
          : [],
        frames: Array.from(
    ...
    },
    {
      ...
      itemPage,
      resourceUri: PLUGIN_PACKAGE_MCP_APP_RESOURCE_URI,
...
async function readMcpAppWebContents(app) {
  return await app.evaluate(
            ...
            try {
              observedMarker = await Promise.race([
                entry.executeJavaScript(
                  "document.body?.dataset?.pluginPackageMcpApp ?? null",
                ),
                new Promise((resolve) =>
    ...
    { marker: PLUGIN_PACKAGE_MCP_APP_MARKER, title: PLUGIN_PACKAGE_MCP_APP_TITLE },
  );
}
