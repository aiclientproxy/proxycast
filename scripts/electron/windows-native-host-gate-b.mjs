...
export function resolveInstalledResourcesRoot(electronExecutable) {
  ...
  if (!existsSync(resourcesRoot) || !statSync(resourcesRoot).isDirectory()) {
    throw new Error(`Installed Electron resources directory is missing: ${resourcesRoot}`);
  }
...
function readInstalledHelper(resourcesRoot) {
  ...
  if (!existsSync(manifestPath)) {
    throw new Error(`Installed desktop resource manifest is missing: ${manifestPath}`);
  }
  ...
  if (!resource || resource.path !== metadata.path) {
    throw new Error("Installed Windows native helper resource is not registered");
  }
  const helperPath = path.resolve(resourcesRoot, metadata.path);
  if (!existsSync(helperPath) || !statSync(helperPath).isFile()) {
    throw new Error(`Installed Windows native helper is missing: ${helperPath}`);
  }
...
class NativeHostClient {
  ...
  constructor(helperPath, timeoutMs) {
    ...
    this.#child.once("exit", (code, signal) => {
      this.#failAll(
        new Error(`Windows native host exited code=${code ?? ""} signal=${signal ?? ""}`),
      );
    });
  }
    ...
    while (true) {
      ...
      if (response.ok === true) {
        ...
        this.#settle(
          response.id,
          null,
          new Error(response.error?.message || "Windows native host request failed"),
        );
      }
    }
  }
...
export async function runWindowsNativeHostGateB(options) {
  const resourcesRoot = resolveInstalledResourcesRoot(options.electronExecutable);
  ...
  try {
    const capabilities = await client.invoke("windows.uiAutomation.capabilities");
    assert(capabilities?.readOnly === true, "native helper must be read-only");
    assert(capabilities?.uiAutomation === "ready", "UI Automation COM is unavailable");
    assert(capabilities?.displayWatcher === "ready", "display watcher is unavailable");
    checks.push({ name: "capabilities", status: "passed" });

    const windows = await client.invoke("windows.window.read");
    assert(Array.isArray(windows?.windows), "window enumeration returned no list");
    ...
    checks.push({ name: "window.read", count: windows.windows.length, status: "passed" });

    const displays = await client.invoke("windows.display.read");
    assert(Array.isArray(displays?.displays) && displays.displays.length > 0, "display enumeration returned no monitors");
    checks.push({ name: "display.read", count: displays.displays.length, status: "passed" });
    ...
    assert(tree?.tree && Number(tree.nodeCount) >= 1, "UI Automation tree is empty");
    checks.push({ name: "uiAutomation.read", nodeCount: tree.nodeCount, status: "passed" });
