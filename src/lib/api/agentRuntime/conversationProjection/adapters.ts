...
export function conversationProjectionEventFromPayload(
  ...
  if (!type) return null;
  const base = { source, ...(eventId ? { event_id: eventId } : {}) };
  switch (type) {
    case "thread_started": {
      const threadId = readString(payload, "thread_id", "threadId", "session_id");
      return threadId
    ...
    case "reasoning_content_delta": {
      const threadId = readString(payload, "thread_id", "threadId", "session_id");
      const turnId = readString(payload, "turn_id", "turnId");
      const itemId = readString(payload, "item_id", "itemId", "reasoning_id", "reasoningId");
      ...
      return threadId && turnId && itemId && value !== undefined && index !== undefined
        ? {
            ...base,
            type: "item_delta",
            thread_id: threadId,
    ...
    case "transport_disconnected": {
      const threadId = readString(payload, "thread_id", "threadId", "session_id");
      return threadId
        ? { ...base, type, thread_id: threadId, reason: readString(payload, "reason") }
        : null;
    }
    default:
      return null;
